...
echo Compiling: ${main} with no stack protection
echo -e "\n##############################################################################\n"
echo Creating IR representation:
clang -O0 -target riscv -mcpu=LowRISC -mriscv=LowRISC $INCLUDES -S -fno-stack-protection  $main -emit-llvm -o $name-tag.ll
echo -e "\toutput to ${name}.ll"
...
echo Running regular version
echo
echo aaaaaaaaaaXWVUTSRQPONMLKJIHGFEDCBA876543210 | spike pk test-simple_input.riscv 
echo
echo Running tagged  version
echo aaaaaaaaaaXWVUTSRQPONMLKJIHGFEDCBA876543210 | spike pk test-simple_input-opt.riscv 
